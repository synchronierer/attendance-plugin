(() => {
  ...
  function init() {
    const form = document.querySelector("#location-form"), body = document.querySelector("#locations"), status = document.querySelector("#status"), heading = document.querySelector("#location-form-heading"), reset = document.querySelector("#reset"), type = form?.elements?.type, typeHelp = document.querySelector("#type-help");
    if (!form || !body || !status || !heading || !reset || !type || !typeHelp) throw new Error("Attendance-Admin: erforderliche Seitenelemente fehlen");
    type.addEventListener("change", event => { typeHelp.textContent = event.target.value === "ARRIVAL" ? "Registriert die Ankunft in der Schule." : "Registriert den aktuellen Arbeits- oder Aufenthaltsort."; });
    ...
    form.onsubmit = async event => { event.preventDefault(); const f = event.currentTarget, fd = new FormData(f), x = Object.fromEntries(fd); x.displayOrder = Number(x.displayOrder); x.active = f.elements.active.checked; if (x.id) x.id = Number(x.id); else delete x.id; try { await postJson("/attendance-location-save", x); status.textContent = "Bereich gespeichert."; f.reset(); f.elements.id.value = ""; f.elements.active.checked = true; heading.textContent = "Neuen Bereich anlegen"; await refresh(); } catch (e) { status.textContent = e.message; } };
    reset.onclick = () => { form.reset(); form.elements.id.value = ""; form.elements.active.checked = true; heading.textContent = "Neuen Bereich anlegen"; };
